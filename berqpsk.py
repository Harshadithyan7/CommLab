...
for i in range(len(ebnodb)):
    ...
    ber[i] = errors / N
    # theoretical ber
    ber_theor[i] = 0.5 * erfc(np.sqrt(ebn0))
plt.subplot(2,2,3)
plt.semilogy(ebnodb, ber,label='Simulated BER')
plt.semilogy(ebnodb, ber_theor,label='Theoretical BER')
